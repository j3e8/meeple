app.service("Person", ["Layer", "RenderUtil", function(Layer, RenderUtil) {
  var Person = {};
  ...
  Person.createBaseLayer = function() {
    ...
    person.composition.forEach(function(c) {
      c.layer = Layer.createBaseLayer(Layer.layers[c.class]);
    });
  ...
  Person.render = function(ctx, person, canvasSize, scale) {
    ...
    person.composition.forEach(function(c) {
      if (Layer.layers[c.class] && Layer.layers[c.class].armature && c.layer && c.layer.size) {
        ctx.save();
